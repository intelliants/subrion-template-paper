    ...
    {if 'account' == $menu.name && $member && $core.config.members_enabled}
        <ul class="b-nav__account">
            ...
            {access object='admin_access'}
                <li><a rel="nofollow" href="{$smarty.const.IA_ADMIN_URL}" target="_blank" title="{lang key='admin_dashboard'}"><span class="fe fe-command"></span><span class="hidden-xs hidden-lg"> {lang key='admin_dashboard'}</span></a></li>
            {/access}
        </ul>
    {else}
        <ul class="b-nav__account">
